...
Meteor.scholarEdits = new Meteor.Collection("scholarEdits");
...
Meteor.scholarEdits.before.insert(function (userId, doc) {
	doc.when = Date.now();
});
...
if (Meteor.isServer) {
	...
	Meteor.publish("scholarEdits", function () {
		return Meteor.scholarEdits.find({});
	});
