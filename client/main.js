Template.scholars.onRendered(function () {
	Meteor.setTimeout(function () {
		...
		$('#scholarsTable_wrapper').prepend('<div id="scholarsTable_length" class="dataTables_length">'+Blaze.toHTMLWithData(Template.scholarsToolbar)+'</div>').show();
		//$('#scholarsTable_filter').html('<input type="search" class="" placeholder="" aria-controls="scholarsTable">');
	}, 1500);
});
...
Template.scholars.events({
	'click #add': function () {
		bootbox.dialog({
			title: "Add Scholar",
			message: Blaze.toHTMLWithData(Template.scholarsAdd),
			onEscape: true,
			closeButton: true,
			buttons: {
				alert: {
					...
					callback: function () {
						...
						if (res) {
							var form = getForm();
							Meteor.scholars.insert({name: form.name, slug: form.slug, born: form.born, died: form.died, });
